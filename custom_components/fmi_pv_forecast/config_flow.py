            ...
            data_schema=vol.Schema(
                {
                    ...
                    vol.Required(CONF_RATED_POWER): vol.All(
                        vol.Coerce(float), vol.Range(min=0.1)
                    ),
                    vol.Optional(CONF_MODULE_ELEVATION, default=DEFAULT_MODULE_ELEVATION): vol.All(
                        vol.Coerce(float), vol.Range(min=0)
                    ),
                    vol.Optional("albedo_preset", default="grass"): vol.In(
                        ["grass", "concrete", "snow", "asphalt", "soil", "water", "custom"]
                    ),
                    vol.Optional(CONF_ALBEDO, default=DEFAULT_ALBEDO): vol.All(
